...
import { Search, X, Filter, List, Grid3X3, CheckSquare } from 'lucide-react';
...
export default function SortFilterBar({
  ...
  return (
    <div className="flex items-center gap-2 mb-4 overflow-x-auto scrollbar-hide -mx-4 px-4">
      {/* Search */}
      {isSearchOpen ? (
        <div className="flex-1 min-w-0 flex items-center gap-2 bg-zinc-800 rounded-lg px-3 py-2">
        ...
        <>
          ...
          <button
            ...
            className="flex-shrink-0 flex items-center gap-2 px-3 py-2 bg-zinc-800 hover:bg-zinc-700 rounded-lg transition"
          >
          ...
          {onFilterClick && (
            ...
            >
              ...
              <span className="text-sm">{filterCount > 0 ? filterCount : 'Filter'}</span>
            </button>
          )}
          ...
          {onLayoutChange && layout && (
            <div className="flex-shrink-0 flex items-center gap-1 bg-zinc-800 rounded-lg p-1">
              <button
          ...
          {onSelectModeChange !== undefined && (
            <button
              ...
              className={`flex-shrink-0 flex items-center gap-1.5 px-3 py-2 rounded-lg transition ${
                isSelectMode
                  ? 'bg-brand-primary text-white'
                  : 'bg-zinc-800 hover:bg-zinc-700 text-gray-400'
              }`}
